/*
 * A driver for alphanetworks_spx70d0_168f ONIE EEPROM
 *
...
struct spx70d0_168f_onie_eeprom_data
{
...
/* Addresses scanned for spx70d0-168f_onie_eeprom */
...
static struct attribute *spx70d0_onie_attributes[] = {
  &sensor_dev_attr_eeprom.dev_attr.attr,
  NULL
};

static const struct attribute_group spx70d0_onie_group = {
  .attrs = spx70d0_onie_attributes,
};
...
static ssize_t onie_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    struct spx70d0_168f_onie_eeprom_data *data = i2c_get_clientdata(client);
...
{
    struct spx70d0_168f_onie_eeprom_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct spx70d0_168f_onie_eeprom_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &spx70d0_onie_group);
    if (status) {
        goto exit_free;
    }
...
{
    struct spx70d0_168f_onie_eeprom_data *data = i2c_get_clientdata(client);
    sysfs_remove_group(&client->dev.kobj, &spx70d0_onie_group);
    kfree(data);
