/*
 * A Power CPLD IPMI kernel dirver for alphanetworks spx70d0-168f
 *
...
#define DRVNAME 					"spx70d0_pwr_cpld"
...
static int spx70d0_pwr_cpld_probe(struct platform_device *pdev);
static int spx70d0_pwr_cpld_remove(struct platform_device *pdev);
...
struct ipmi_pwr_resp_data {
	unsigned char   		status[32];

};

struct spx70d0_pwr_cpld_data {
	...
	struct ipmi_pwr_resp_data 		ipmi_resp[2]; 			/* 0: PSU1, 1: PSU2 */
	unsigned char    				ipmi_resp_cpld;         /* PWR CPLD */
	unsigned char 					ipmi_tx_data[4];
};

struct spx70d0_pwr_cpld_data *data = NULL;

static struct platform_driver spx70d0_pwr_cpld_driver = {
	.probe      = spx70d0_pwr_cpld_probe,
	.remove     = spx70d0_pwr_cpld_remove,
	.driver     = {
		.name   = DRVNAME,
		.owner  = THIS_MODULE,
	},
...
enum spx70d0_pwr_cpld_sysfs_attrs {
	/* psu attributes */
	PSU_ATTR(1),
	PSU_ATTR(2),
	NUM_OF_PSU_ATTR,
...
static struct attribute *spx70d0_pwr_cpld_attributes[] = {
	/* psu attributes */
	DECLARE_PSU_ATTR(1),
	DECLARE_PSU_ATTR(2),
	/* pwr cpld attributes */
...
static const struct attribute_group spx70d0_pwr_cpld_group = {
	.attrs = spx70d0_pwr_cpld_attributes,
};
...
static struct spx70d0_pwr_cpld_data *
spx70d0_pwr_cpld_update_device(struct device_attribute *da)
{
...
static struct spx70d0_pwr_cpld_data *
spx70d0_pwr_cpld_update_cpld_ver(unsigned char cpld_addr)
{
	int status = 0;
			...
			char *buf)
{
	...
	data = spx70d0_pwr_cpld_update_device(da);
	if (!data->valid[pid]) {
		error = -EIO;
		goto exit;
	}
	
	DEBUG_PRINT("spx70d0_168f_pwr_cpld show_psu: data->ipmi_resp[%d].status[0]:0x%x",
				pid, data->ipmi_resp[pid].status[0]);

	mask = 0x1 << PSU_STATUS_BIT_PRESENT;
	present = !(!!(data->ipmi_resp[pid].status[0] & mask)); /* HW present:0, not present:1. need to convert */

	DEBUG_PRINT("spx70d0_168f_pwr_cpld show_psu: pid:%d, present:%d \n",
				pid, present);
	...
	DEBUG_PRINT("spx70d0_168f_pwr_cpld show_psu: pid:%d, value:%d \n",
					pid, value);
                ...
                struct device_attribute *da, char *buf)
{
	...
	data = spx70d0_pwr_cpld_update_cpld_ver(cpld_addr);
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
	...
	DEBUG_PRINT("spx70d0_168f_pwr_cpld show_cpld_version: data->ipmi_resp_cpld:0x%x", data->ipmi_resp_cpld);
...
static int spx70d0_pwr_cpld_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &spx70d0_pwr_cpld_group);
	if (status) {
		goto exit;
	}
...
static int spx70d0_pwr_cpld_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &spx70d0_pwr_cpld_group);
	return 0;
}

static int __init spx70d0_pwr_cpld_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct spx70d0_pwr_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&spx70d0_pwr_cpld_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&spx70d0_pwr_cpld_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit spx70d0_pwr_cpld_exit(void)
{
...
module_init(spx70d0_pwr_cpld_init);
module_exit(spx70d0_pwr_cpld_exit);

MODULE_AUTHOR("Alpha-SID6");
MODULE_DESCRIPTION("Alphanetworks spx70d0-168f Power CPLD driver");
MODULE_LICENSE("GPL");
