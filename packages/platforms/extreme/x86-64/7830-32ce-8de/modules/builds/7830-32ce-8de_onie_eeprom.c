/*
 * A driver for extremenetworks_7830_32ce_8de ONIE EEPROM
 *
...
struct extreme7830_32ce_8de_onie_eeprom_data
{
...
/* Addresses scanned for extreme7830-32ce-8de_onie_eeprom */
static const unsigned short normal_i2c[] = { 0x55, I2C_CLIENT_END };

enum extreme7830_32ce_8de_onie_eeprom_sysfs_attributes {
    ONIE_RW,
};

static SENSOR_DEVICE_ATTR(eeprom, (0660), onie_read, onie_write, ONIE_RW);

static struct attribute *extreme7830_onie_attributes[] = {
  &sensor_dev_attr_eeprom.dev_attr.attr,
  NULL
};

static const struct attribute_group extreme7830_onie_group = {
  .attrs = extreme7830_onie_attributes,
};
...
static ssize_t onie_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    struct extreme7830_32ce_8de_onie_eeprom_data *data = i2c_get_clientdata(client);
...
{
    struct extreme7830_32ce_8de_onie_eeprom_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct extreme7830_32ce_8de_onie_eeprom_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &extreme7830_onie_group);
    if (status) {
        goto exit_free;
    }
...
{
    struct extreme7830_32ce_8de_onie_eeprom_data *data = i2c_get_clientdata(client);
    sysfs_remove_group(&client->dev.kobj, &extreme7830_onie_group);
    kfree(data);
